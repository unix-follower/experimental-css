...
interface ParticleParams extends CircleParams {
...
class Particle extends Circle {
  ...
  radians: number
  velocity: number
  distanceFromCenter: Vector2
  lastMousePosition: Vector2

  constructor({ x, y, radius, color, mousePosition }: ParticleParams) {
    ...
    this.velocity = 0.02
    this.distanceFromCenter = { x: randomIntFromRange(50, 320), y: randomIntFromRange(60, 65) }
  ...
  update(ctx: CanvasRenderingContext2D) {
    ...
    // Drag effect
    this.lastMousePosition.x += (this.mousePosition.x - this.lastMousePosition.x) * 0.05
    this.lastMousePosition.y += (this.mousePosition.y - this.lastMousePosition.y) * 0.05
    ...
    if (
      this.mousePosition.x - this.x < 30 &&
      this.mousePosition.x - this.x > -30 &&
      this.mousePosition.y - this.y < 30 &&
      this.mousePosition.y - this.y > -30
    ) {
      if (this.radiusData.size < this.radiusData.max) this.radiusData.size += 1
    } else if (this.radiusData.size > this.radiusData.min) {
      this.radiusData.size -= 0.2
      this.radiusData.size = Math.max(1, this.radiusData.size)
    }
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ctx.beginPath()
    ctx.arc(this.x, this.y, this.radiusData.size, 0, Math.PI * 2, false)
...
function init(canvas: HTMLCanvasElement, mousePosition: Vector2, colors: string[]) {
...
function main() {
  ...
  let randomColors = colors[29]
  const background = "#131716"
  ...
  let particles = init(canvas, mousePosition, randomColors)
  addEventListener("resize", () => {
    canvas.width = innerWidth
    canvas.height = innerHeight

    particles = init(canvas, mousePosition, randomColors)
  })

  addEventListener("mousedown", () => {
    const random = randomIntFromRange(0, 99)
